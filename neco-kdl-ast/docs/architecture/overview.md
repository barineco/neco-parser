...
graph TD
    ...
    AST["neco-kdl-ast ( AST layer )<br/>NsidPath / CrossRef / StructuredName<br/>StructuredNode / Document<br/>KDL helper"]
...
`neco-kdl-ast` は KDL 固有の `NsidPath` / `CrossRef` / layout 判定 / dot-chain helper を保持しつつ、`Document` と `StructuredNode<'a>` に `neco-ast` の共有 trait を実装する。JSON / TOML / YAML / JSON5 / XML / plist は各 `*-ast` crate が同じ trait を実装する。
