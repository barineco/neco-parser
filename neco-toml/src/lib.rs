...
fn parse_lines(input: &str, sep: &str) -> Result<TomlValue, ParseError> {
    ...
    for (line_no, raw) in input.lines().enumerate() {
        ...
        if value.is_empty() {
            fields.push((key, TomlValue::List(Vec::new())));
        } else {
            fields.push((key, parse_scalar(value)));
        }
    }
    Ok(TomlValue::Map(fields))
}

fn push_list_item(fields: &mut Vec<(String, TomlValue)>, key: String, value: TomlValue) {
    if let Some((_, TomlValue::List(items))) = fields.iter_mut().rev().find(|(k, _)| *k == key) {
        items.push(value);
    } else {
        fields.push((key, TomlValue::List(vec![value])));
    }
}

#[allow(dead_code)]
fn parse_json5_like(input: &str) -> Result<TomlValue, ParseError> {
...
#[allow(dead_code)]
fn parse_xml_like(input: &str) -> Result<TomlValue, ParseError> {
    let mut fields = Vec::new();
    ...
    while let Some(open) = rest.find('<') {
        ...
        if after[..end_name].trim_end().ends_with('/') {
            fields.push((name, TomlValue::String(String::new())));
            continue;
        }
...
";
    ...
    fn case_01() {
        ...
        assert!(matches!(v, TomlValue::Map(_)));
        assert!(matches!(v, TomlValue::Map(_)));
    }
    #[test]
    fn case_02() {
    ...
    fn map_has_string(value: &TomlValue, key: &str, expected: &str) -> bool {
        match value {
            TomlValue::Map(fields) => fields
                .iter()
                .any(|(k, v)| k == key && matches!(v, TomlValue::String(s) if s == expected)),
            _ => false,
        }
    }

    fn map_len(value: &TomlValue) -> usize {
        match value {
            TomlValue::Map(fields) => fields.len(),
            _ => 0,
        }
    }
}
