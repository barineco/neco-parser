...
pub fn parse(input: &str) -> Result<YmlValue, ParseError> {
    parse_lines(input, ":")
}

fn parse_lines(input: &str, sep: &str) -> Result<YmlValue, ParseError> {
    ...
    for (line_no, raw) in input.lines().enumerate() {
        ...
        let Some((k, v)) = line.split_once(sep).or_else(|| line.split_once('=')) else {
        ...
        if value.is_empty() {
            fields.push((key, YmlValue::List(Vec::new())));
        } else {
            fields.push((key, parse_scalar(value)));
        }
    }
    Ok(YmlValue::Map(fields))
}

fn push_list_item(fields: &mut Vec<(String, YmlValue)>, key: String, value: YmlValue) {
    if let Some((_, YmlValue::List(items))) = fields.iter_mut().rev().find(|(k, _)| *k == key) {
        items.push(value);
    } else {
        fields.push((key, YmlValue::List(vec![value])));
    }
}

#[allow(dead_code)]
fn parse_json5_like(input: &str) -> Result<YmlValue, ParseError> {
...
#[allow(dead_code)]
fn parse_xml_like(input: &str) -> Result<YmlValue, ParseError> {
    let mut fields = Vec::new();
    ...
    while let Some(open) = rest.find('<') {
        ...
        if after[..end_name].trim_end().ends_with('/') {
            fields.push((name, YmlValue::String(String::new())));
            continue;
        }
...
fn parse_scalar(raw: &str) -> YmlValue {
    let s = raw
        .trim()
        .trim_end_matches(',')
        .trim_matches('"')
    ...
    if s.eq_ignore_ascii_case("true") {
        return YmlValue::Bool(true);
    }
    if s.eq_ignore_ascii_case("false") {
        return YmlValue::Bool(false);
    }
    if s.eq_ignore_ascii_case("null") || s == "~" {
        return YmlValue::Null;
    }
    if s.starts_with('[') && s.ends_with(']') {
        let inner = &s[1..s.len() - 1];
        return YmlValue::List(
            inner
                .split(',')
    ...
    if let Ok(n) = s.parse::<f64>() {
        return YmlValue::Number(n);
    }
    YmlValue::String(s.to_string())
}
...
";
    ...
    fn case_01() {
        ...
        assert!(matches!(v, YmlValue::Map(_)));
        assert!(matches!(v, YmlValue::Map(_)));
    }
    #[test]
    fn case_02() {
    ...
    fn map_has_string(value: &YmlValue, key: &str, expected: &str) -> bool {
        match value {
            YmlValue::Map(fields) => fields
                .iter()
                .any(|(k, v)| k == key && matches!(v, YmlValue::String(s) if s == expected)),
            _ => false,
        }
    }

    fn map_len(value: &YmlValue) -> usize {
        match value {
            YmlValue::Map(fields) => fields.len(),
            _ => 0,
        }
    }
}
